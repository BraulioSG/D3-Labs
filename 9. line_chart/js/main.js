...
d3.json("data/example.json").then((data) => {
    ...
    y.domain([765000, 790000]);
    x.domain([d3.min(data.map(d => d.year)), d3.max(data.map(d=>d.year))]);
    ...
    // Add line to chart
    // TODO: add line path
    g.append("path").attr("class", "line").attr("fill", "none").attr("stroke", "grey").attr("stroke-width", "3px").attr("d", line(data));
