...
d3.json("data/revenues.json").then(data => {
    ...
    const names = data.map(b => b.month);

    const x = d3.scaleBand().domain(names).range([0,400]).paddingInner(0.3).paddingOuter(0.3);
    const y = d3.scaleLinear().domain([0, maxRevenue]).range([400, 0]);

    const rects =group.selectAll("rect").data(data);
    rects.enter().append("rect").
        attr("x", data => {return x(data.month)} ).
        attr("y", data => {return y(data.revenue)}).
        attr("width", 30).
        attr("height", data => SVG_HEIGHT - y(data.revenue)).
        attr("fill", "yellow");
